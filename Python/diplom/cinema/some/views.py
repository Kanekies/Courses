...
class ShowList(ListView):
    ...
    def get_queryset(self):
        ...
        if param_date:
            ...
            queryset = queryset.filter(show_time_start__gt=now)
        queryset = queryset.annotate(free_places=F('place__size')-F('busy'))
        return queryset
...
class OrderCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        if show_busy + user_amount > show.place.size:
            ...
            return HttpResponseRedirect(reverse('main'))
        messages.info(self.request, 'Thank you 4 order')
...
class PlaceListView(PermissionRequiredMixin, ListView):
    ...
    queryset = Place.objects.exclude(shows__busy__gt=0)
