...
class ShowSerializer(serializers.ModelSerializer):
    ...
    def validate(self, cleaned_data):
        ...
        if place and show_start and show_end:
            ...
            # self.instance - чтобы не проверялось с самим собой
        ...
        # если данные валидны, то просто создает
        return cleaned_data

    # pop - берет значение ключа, удаляет из словаря этот ключ со значением
    # не храним два одинаковых значения в двух местах
    # из validated_data ЗАБИРАЕМ вложенные объекты зала и фильма и передаем аргументами в функцию создания объекта
    # сеанса, т. к. джанго сериалайзеры по дефолту не распознают вложенные объекты
    def create(self, validated_data):
    ...
    # instance - объект сеанса, который хотим обновить, обновляем вложенные объекты
    # фильма и зала(джанго не распознает по дефолту)
